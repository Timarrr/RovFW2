...
PDRegulator::PDRegulator(int kP, int kD): kP(kP), kD(kD), lastErr(0), lastTime(0){}

int PDRegulator::eval(int data, int target){
   int err = target - data;
   int u = kP * (err + kD * ((lastErr - err)/(lastTime - millis()))); //kP(error + kD(dError/dt))
   lastErr = err;
   lastTime = millis();
   return u;
}
...
void RovRegulators::evaluate(RovControl &ctrl, RovAuxControl &auxCtrl, RovTelemetry &tele){
    int uD=0,uY=0,uR=0,uP=0;
    if (auxCtrl.auxFlags.eDepth) depthReg.eval(tele.depth * 100, auxCtrl.dDepth);
    if (auxCtrl.auxFlags.eYaw) yawReg.eval(tele.yaw * 100, auxCtrl.dYaw * 100);
    if (auxCtrl.auxFlags.eRoll) rollReg.eval(tele.roll * 100, auxCtrl.dRoll * 100);
    if (auxCtrl.auxFlags.ePitch) pitchReg.eval(tele.pitch * 100, auxCtrl.dPitch * 100);
